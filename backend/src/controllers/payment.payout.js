...
// NOTE: UNPAY integration is legacy/broken and disabled in production.
// DO NOT import or call unpayPayout until a new gateway is integrated.

/**
 * Initiate a payout
 */
const initiatePayout = async (req, res) => {
    // PRODUCTION SAFETY: payout via legacy UNPAY/dummy is blocked.
    // Remove this block only when a production-ready payout adapter is implemented.
    if (process.env.NODE_ENV === 'production') {
        return res.status(503).json({
            success: false,
            code: 'PAYOUT_PROVIDER_NOT_CONFIGURED',
            message: 'Payout is not available. No production payout provider is configured.'
        });
    }
    ...
    try {
      ...
      if (user.MerchantDetails.payout_merchant_name === 'unpay') {
        // UNPAY is disabled — do not call legacy integration
        logger.error('Attempted UNPAY payout — provider is disabled', { user_id: user_id });
        return res.status(503).json({
          success: false,
          code: 'PAYOUT_PROVIDER_NOT_CONFIGURED',
          message: 'Legacy payout provider is disabled. Contact administrator.'
        });
      }
